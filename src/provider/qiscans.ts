...
export const qiscans: Provider = {
    ...
    async fetchChapter(slug: string, chapter: number): Promise<ChapterData> {
        ...
        const prev = chapter > 1 ? `https://qimanga.com/series/${slug}/chapter-${chapter - 1}` : null;
        const next = `https://qimanga.com/series/${slug}/chapter-${chapter + 1}`;
