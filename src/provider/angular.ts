...
export function createAngularProvider(site: keyof typeof SITE_CONFIG): Provider {
    ...
    async function fetchAngularChapter(slug: string, chapterId: string): Promise<import('./types').ChapterData | null> {
        ...
        if (data.images.length === 0) throw new Error('Chapter response contained no images');
    ...
    async function fetchAngularChaptersNewestFirst(slug: string): Promise<ChapterMeta[]> {
        ...
        while (hasMore) {
            ...
            for (const item of data.data) chapters.push({ chapterId: item.slug });
    ...
    function readerUrl(slug: string, chapterId: string, imageIndex?: string): string {
    ...
    return {
        ...
        fetchHome: workerHome(site),
        loadChapter: chapterLoader(fetchAngularChapter, seriesUrl),
        resolveHomeDestination: homeDestinationResolver({
            fetchChapter: fetchAngularChapter,
            fetchChaptersNewestFirst: fetchAngularChaptersNewestFirst,
            readerUrl,
            seriesUrl,
        }),
        fetchChaptersNewestFirst: fetchAngularChaptersNewestFirst,
