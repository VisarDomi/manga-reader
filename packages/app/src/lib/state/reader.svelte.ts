...
export class ReaderState {
    ...
    private windowHydrates = new Map<string, Promise<ChapterLoadResult>>();
    ...
    reconcileReaderWindow(viewport: { scrollTop: number; clientHeight: number; clientWidth: number; chapterId?: string | null; physicalWindowStart?: number; projectionEpoch?: number }, source: ReaderWindowSource): ReaderWindowReconcileResult | null {
        ...
        this.schedulePhysicalWindowHydrates(manga, plan.warmCandidates, source);
        return {
    ...
    private commitWindowFrame({
        ...
        if (projectionChanged && bumpProjectionEpoch) {
    ...
    private schedulePhysicalWindowHydrates(
        manga: Manga,
        ...
        const capacity = Math.max(0, 3 - this.windowHydrates.size);
        if (capacity === 0) return;

        const queued = candidates
            .filter(candidate => {
                const chapterId = candidate.chapter.id;
                return !this.chapterDataById.has(chapterId)
                    && !this.windowFetches.has(chapterId)
                    && !this.windowHydrates.has(chapterId);
            })
            .slice(0, capacity);

        for (const candidate of queued) {
            const chapterId = candidate.chapter.id;
            this.log.emit('reader-window-hydrate-start', {
                source,
                mangaId: manga.id,
                chapterId,
                ...
                hydrateCount: this.windowHydrates.size + 1,
            });
            void this.fetchChapterMetadata(manga, candidate.chapter)
                .then(result => {
                    if (this.windowEpoch !== windowEpoch || this.loadEpoch !== loadEpoch || result.kind === 'stale') return;
                    if (result.kind === 'failed') {
                        this.log.emit('reader-window-hydrate-failed', {
                            source,
                            mangaId: manga.id,
                            chapterId,
    ...
    private scheduleWindowFetches(manga: Manga, candidates: WindowCandidate[], source: ReaderWindowSource): void {
        ...
        for (const candidate of candidates) {
            ...
            const ready = this.chapterDataById.get(candidate.chapter.id);
            if (!slot || slot.slotState === 'ready' || !ready) continue;
            this.log.emit('reader-window-local-hit', {
                source,
                mangaId: manga.id,
            ...
            this.applyReadyWindowChapter(manga, ready, source, candidate.side === 'current' ? 'current' : 'window');
        }
    ...
    private async fetchWindowChapter(manga: Manga, candidate: WindowCandidate, windowEpoch: number, loadEpoch: number, source: ReaderWindowSource): Promise<void> {
        try {
            const result = await this.fetchChapterMetadata(manga, candidate.chapter);
    ...
    private fetchChapterMetadata(manga: Manga, chapter: ChapterMeta): Promise<ChapterLoadResult> {
        const existing = this.windowHydrates.get(chapter.id);
        if (existing) return existing;

        const request = this.loadChapter({ manga, chapter })
            .finally(() => {
                if (this.windowHydrates.get(chapter.id) === request) {
                    this.windowHydrates.delete(chapter.id);
                }
            });
        this.windowHydrates.set(chapter.id, request);
        return request;
    }

    private readyChapterFromResult(chapter: LoadedChapter, viewportWidth: number): LoadedChapter {
        return {
            ...chapter,
            slotState: 'ready',
            estimatedHeight: this.estimateLoadedChapterHeight(chapter.pages, viewportWidth),
        };
    }
    ...
    private applyReadyWindowChapter(
        ...
        const physicalScrollTop = Math.max(0, this.lastWindowScrollTop - this.physicalWindowStart);
        ...
        let frame: ReturnType<typeof this.commitWindowFrame>;
        if (changesHeightAboveAnchor) {
            const previousPhysicalWindowStart = this.physicalWindowStart;
            const nextPhysicalWindowStart = Math.max(0, this.physicalWindowStart + heightDelta);
            const actualStartDelta = nextPhysicalWindowStart - previousPhysicalWindowStart;
            const logicalScrollTop = nextPhysicalWindowStart + physicalScrollTop;
            this.lastWindowScrollTop = logicalScrollTop;
            const plan = this.windowManager.plan({
                chapterList: this.chapterList,
                loadedChapters: this.plannerChapters(),
                ...
                clientHeight: Math.max(1, this.layoutViewportHeight()),
                direction: 'idle',
                heightRevision: this.heightRevision,
                estimateChapterHeight: (chapterId, width) => this.estimateChapterHeight(chapterId, width),
                preserveLoadedSlots: true,
            });
            frame = this.commitWindowFrame({
                source,
                mangaId: manga.id,
                ...
                physicalWindowStart: plan.physicalWindowStart,
                physicalScrollTop,
                physicalHeight: Math.max(plan.physicalHeight, this.layoutViewportHeight()),
                slots: plan.nextSlots,
                bumpProjectionEpoch: false,
            });
            this.log.emit('reader-window-anchor-projection', {
                source,
                mangaId: manga.id,
                chapterId: readyChapter.id,
                anchorChapterId,
                heightDelta: Math.round(heightDelta),
                physicalStartDelta: Math.round(actualStartDelta),
                physicalWindowStart: Math.round(frame.projectionChanged ? plan.physicalWindowStart : this.physicalWindowStart),
                physicalScrollTop: Math.round(physicalScrollTop),
                frameEpoch: frame.epoch,
            });
        } else {
