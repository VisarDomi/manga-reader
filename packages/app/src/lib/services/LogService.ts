export type LogEvent =
    ...
    | { event: 'reader-image-lifecycle'; key: string; ok: boolean; policy: 'critical' | 'preload'; candidateIndex: number; candidateTotal: number; host: string; naturalWidth: number; naturalHeight: number; displayWidth: number; displayHeight: number; totalMs: number; error?: string }
