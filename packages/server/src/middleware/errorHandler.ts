...
import { UpstreamError, UpstreamBodyError, CloudflareError, ParseError } from '../utils/proxyFetch.js';
...
function formatOptionalMeta(meta: ProxyFetchMeta | null): string {
  return meta ? formatMeta(meta) : 'url=browser-session';
}
...
export function errorHandler(err: Error, req: Request, res: Response, _next: NextFunction): void {
  ...
  } else if (err instanceof UpstreamBodyError) {
    console.error(`[${req.path}] upstream-body apiStatus=${err.upstreamStatus} message="${err.upstreamMessage}" ${formatOptionalMeta(err.meta)}`);
    res.status(err.status).json({ error: err.message, status: err.status, upstreamStatus: err.upstreamStatus });
