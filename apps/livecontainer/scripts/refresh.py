...
def app_profiles(app, expected):
    ...
    # LC's supported single-profile packaging uses the exact host profile in both
    # launch helpers. Validate their identities and bytes before deduplicating the
    # one renewal deadline; never silently ignore unrelated or expired profiles.
    helper_ids = {
        'LaunchAppExtension.appex': 'com.kdt.livecontainer.AVQL5DLWLT.LaunchAppExtension',
        'ShareExtension.appex': 'com.kdt.livecontainer.AVQL5DLWLT.ShareExtension',
    }
    if set(path.parent.name for path in paths[1:]) != set(helper_ids):
        raise RuntimeError('Unexpected LiveContainer helper profile set')
    for path in paths[1:]:
        info = plistlib.loads((path.parent / 'Info.plist').read_bytes())
        if info.get('CFBundleIdentifier') != helper_ids[path.parent.name] or path.read_bytes() != paths[0].read_bytes():
            raise RuntimeError('LiveContainer helper must use the exact main profile and approved bundle ID')
    paths = paths[:1]
