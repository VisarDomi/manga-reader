...
import argparse, hashlib, json, pathlib, plistlib, re, shlex, subprocess
...
q = shlex.quote
label = 'com.visar.'+args.provider+'-reader-build'; app = remote + '/build/'+args.provider+suffix+'/Release-iphoneos/'+provider['productName']+'.app'
if args.command == 'sync':
    build = ROOT / 'build'; build.mkdir(exist_ok=True)
    plist = {'Label':label,'ProgramArguments':['/usr/bin/caffeinate','-i','-t','1800','/bin/bash',remote+'/scripts/build.sh',args.provider],'EnvironmentVariables':{'DEVELOPMENT_TEAM':config['signingTeam'],'READER_BUNDLE_SUFFIX':suffix,'DEVELOPMENT_DEVICE':config['device']},'RunAtLoad':True,'StandardOutPath':remote+'/gui-build.log','StandardErrorPath':remote+'/gui-build.log'}
